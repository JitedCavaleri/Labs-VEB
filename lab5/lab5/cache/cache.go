...
func KeyItemsList(userID uint, page, limit int) string {
	return fmt.Sprintf("%s:items:user:%d:list:page:%d:limit:%d", AppPrefix, userID, page, limit)
}
...
func KeyItemsListPatternForUser(userID uint) string {
	return fmt.Sprintf("%s:items:user:%d:list:*", AppPrefix, userID)
}

// KeyItemByID — кеш одного item.
func KeyItemByID(userID, itemID uint) string {
	return fmt.Sprintf("%s:items:user:%d:item:%d", AppPrefix, userID, itemID)
}

// KeyUserProfile — кеш профиля для /auth/whoami.
func KeyUserProfile(userID uint) string {
	return fmt.Sprintf("%s:users:profile:%d", AppPrefix, userID)
}
...
func KeyAccessJTI(userID uint, jti string) string {
	return fmt.Sprintf("%s:auth:user:%d:access:%s", AppPrefix, userID, jti)
}

// KeyAccessJTIPatternForUser — все access-JTI пользователя (для logout-all).
func KeyAccessJTIPatternForUser(userID uint) string {
	return fmt.Sprintf("%s:auth:user:%d:access:*", AppPrefix, userID)
}

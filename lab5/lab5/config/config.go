...
type Config struct {
	...
	// --- Redis / Cache (Лаба 5) ---
	// Адрес Redis в формате host:port (например, redis:6379 внутри docker-compose).
	RedisAddr string
	// Пароль для AUTH. Обязателен по условию: Redis должен быть защищён паролем.
	RedisPassword string
	// Номер логической БД Redis (0..15).
	RedisDB int
	// TTL по умолчанию для значений, если он не передан в Set явно.
...
func LoadConfig() *Config {
	cfg := &Config{
		...
		// Redis: host собирается из REDIS_HOST + REDIS_PORT (как в примере из задания).
